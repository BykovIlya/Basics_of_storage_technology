<template>
    <div>
        <b-navbar toggleable="md" type="dark" :variant="background" fixed="top" sticky>
            <b-navbar-brand>{{ $t("message.studios.title") }}</b-navbar-brand>
            <b-navbar-nav class = "ml-auto">
                <b-button variant="success" @click="showModal(false, false)">{{ $t("message.studios.add") }}</b-button>
            </b-navbar-nav>
        </b-navbar>
        ...
        <studio ref="studio" v-on:refresh="refreshTable"></studio>
    ...
    export default {
        ...
        data() {
            return {
                ...
                fields: [
                    ...
                    {
                        key: 'name',
                        label: this.$t('message.studios.table.name'),
                        sortable: true,
                    },
                    {
                        key: 'year',
                        label: this.$t('message.studios.table.year'),
                        sortable: true
                    },
                    {
                        key: 'all_films',
                        label: this.$t('message.studios.table.all_films'),
                        sortable: true
                    },
                    {
                        key: 'action',
                        label: this.$t('message.studios.table.actions'),
                        class:'actions-width'
                    },
                ],
                currentPage: 1,
        ...
        methods: {
            ...
            showModal(row, isRead, isNew) {
                this.$refs.studio.showModal(row, isRead, isNew)
            },
            showModalDelete(row) {
                this.$refs.studio.showModalDelete(row)
            },
        }
    }
</script>
